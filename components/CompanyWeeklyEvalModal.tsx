...
interface CompanyWeeklyEvalModalProps {
  registration: Registration;
  appsScriptUrl: string;
  onClose: () => void;
}

export default function CompanyWeeklyEvalModal({ registration, appsScriptUrl, onClose }: CompanyWeeklyEvalModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(appsScriptUrl, {
        method: 'POST',
        body: JSON.stringify({
          action: 'submitCompanyEvaluation',
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
        <div className="bg-white rounded-2xl w-full max-w-md p-6 text-center animate-slide-up shadow-2xl">
          ...
          <p className="text-slate-600 text-sm mb-6">
            Cảm ơn doanh nghiệp đã phản hồi cho sinh viên <strong>{registration.studentName}</strong> (Tuần {weekNumber}).
          </p>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl animate-slide-up">
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div>
            <h2 className="text-lg font-bold text-slate-800">Đánh giá Sinh viên Thực tập</h2>
            <p className="text-sm text-slate-500 mt-0.5">
              {registration.studentName} - MSSV: {registration.studentId}
            </p>
          </div>
